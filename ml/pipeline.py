...
==========================================
Steps:
  1. Data cleaning & preprocessing (NLTK)
  2. Feature extraction (TF-IDF + BoW)
  3. Model training (LR, NB, RF, SVM)
  4. Evaluation (accuracy, precision, recall, F1)
  5. Best-model selection + serialisation
"""
...
def build_vectorizers():
    """Return a dict of vectoriser configs to try."""
    return {
...
# ── Synthetic data (fallback when Kaggle data is unavailable) ─────────────────

def generate_synthetic_data(n: int = 3000) -> pd.DataFrame:
    """
    Generates a realistic synthetic dataset for demo/testing purposes.
    In production, replace with the real Kaggle dataset loader below.
    """
    ...
    fake_templates = [
        ...
        "EXCLUSIVE: {topic} cure found by local {person} – Big Pharma SUPPRESSES it!",
        "ALERT: {country} invades {place} – World War III has OFFICIALLY started!",
        "BOMBSHELL: {celebrity} arrested for {crime} – you won't believe what happened next!",
        "TOP SECRET: {politician} caught accepting bribes from {country} elites!",
    ...
    real_templates = [
        ...
        "City council approves {n}-million-dollar infrastructure bill for road repairs.",
        "Tech company reports {n}% revenue increase in Q{q} earnings call.",
        "University researchers develop new method to detect {topic} earlier.",
        "Health officials confirm {n} new cases of {disease} in {region}.",
        "Climate summit concludes with {n} nations agreeing to reduce emissions by {n}%.",
    ]
    ...
    fillers = {
        ...
        "topic": ["vaccines", "climate change", "5G networks", "water fluoridation"],
        "politician": ["a senator", "a world leader", "a cabinet member"],
        "crime": ["fraud", "espionage", "money laundering"],
        "person": ["farmer", "teacher", "retired nurse"],
    ...
    fake_texts = [fill(rng.choice(fake_templates)) + " " +
                  " ".join(rng.choice(["claim", "exposed", "hidden", "truth",
                                        "secret", "mainstream", "elite", "deep state",
                                        "cover", "lies", "shocking", "unbelievable"])
                            for _ in range(rng.integers(8, 20)))
                  for _ in range(n // 2)]

    real_texts = [fill(rng.choice(real_templates)) + " " +
                  " ".join(rng.choice(["according", "reported", "announced", "confirmed",
                                        "study", "officials", "percent", "increased",
                                        "analysis", "data", "research", "published"])
                            for _ in range(rng.integers(8, 20)))
                  for _ in range(n // 2)]
...
# ── Kaggle data loader (use when you have the CSVs) ───────────────────────────

def load_kaggle_data(fake_path: str, real_path: str) -> pd.DataFrame:
    """
    Load the standard Kaggle Fake News dataset.
    Expects columns: title, text  (plus a label derived from filename).
    """
...
def train(df: pd.DataFrame | None = None):
    ...
    # 4. Grid search: vectorisers × models
    ...
    for vec_name, vec in vectorizers.items():
        for model_name, clf in models.items():
            # MultinomialNB only accepts non-negative features → skip char TF-IDF
...
class FakeNewsPredictor:
    ...
    def __init__(self, model_path: str | None = None):
        ...
        self.preprocessor = arts["preprocessor"]
    ...
    def predict(self, text: str) -> dict:
        ...
        if hasattr(clf, "predict_proba"):
            ...
            confidence = float(1 / (1 + np.exp(-abs(df_val))))  # sigmoid
        else:
            confidence = 1.0
